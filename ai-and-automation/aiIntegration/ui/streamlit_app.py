...
def render_ab_testing():
    ...
    if st.session_state.ab_test_results:
        ...
        has_result_a = "content" in latest_test.get("result_a", {})
        has_result_b = "content" in latest_test.get("result_b", {})
        
        if not has_result_a or not has_result_b:
            st.error("Test results are incomplete. Try running the test again or check the debug information.")
            return
