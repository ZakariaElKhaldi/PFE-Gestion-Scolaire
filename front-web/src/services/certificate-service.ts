...
export const certificateService = {
  ...
  async getStudentCertificates(): Promise<Certificate[]> {
    const { data } = await apiClient.get<Certificate[]>('/certificates/student');
    return data;
  },
  ...
  async getCertificateById(certificateId: string): Promise<Certificate> {
    const { data } = await apiClient.get<Certificate>(`/certificates/${certificateId}`);
    return data;
  },
  ...
  async verifyCertificate(verificationId: string): Promise<CertificateVerification> {
    const { data } = await apiClient.get<CertificateVerification>(`/certificates/verify/${verificationId}`);
    return data;
  },
